...
# Pos代入によるズレで自身がブロックに埋まっていたら後ろに少し下がる
    execute at @s align xyz unless block ~ ~ ~ #marker_motion:no_collision at @s rotated as @e[tag=MarkerMotion.this,limit=1] run tp @s ^ ^ ^-0.01
...
# ブロック接触チェック
    ...
    # 接触検知したらいろいろするやつ
        execute if score #MarkerMotion.BlockCheck neac_value matches 1 at @s align x run tp @s ~0.989 ~ ~
        execute if score #MarkerMotion.BlockCheck neac_value matches 2 at @s align x run tp @s ~0.011 ~ ~
        execute if score #MarkerMotion.BlockCheck neac_value matches 3 at @s align y run tp @s ~ ~0.989 ~
        execute if score #MarkerMotion.BlockCheck neac_value matches 4 at @s align y run tp @s ~ ~0.011 ~
        execute if score #MarkerMotion.BlockCheck neac_value matches 5 at @s align z run tp @s ~ ~ ~0.989
        execute if score #MarkerMotion.BlockCheck neac_value matches 6 at @s align z run tp @s ~ ~ ~0.011
        ...
        # ここが動いたら以降の処理(タグつけたりが動かなくなる仕様)
            ...
            execute store result score #MarkerMotion.Temporary1 neac_value run data get storage neac: _.MarkerMotion.bounce.count 1
