...
# バウンス回数が無限(-1)、0でない時にバウンス回数を1減らす
    execute unless data storage neac: _.MarkerMotion.bounce{count:-1} unless data storage neac: _.MarkerMotion.bounce{count:0} store result storage neac: _.MarkerMotion.bounce.count int 1 run scoreboard players remove #MarkerMotion.Bounce.Count neac_value 1
