...
# 衝突方向に応じて計算する用の値をもらってくる
    execute unless score #MarkerMotion.BlockCheck neac_value matches 3..4 store result score #MarkerMotion.Bounce.Rotation0 neac_value run data get entity @e[tag=MarkerMotion.this,limit=1] Rotation[0] 100
    # X軸方向はいろいろあるので必ずもらう
        execute store result score #MarkerMotion.Bounce.Rotation1 neac_value run data get entity @e[tag=MarkerMotion.this,limit=1] Rotation[1] 100
...
# 反発係数があったらそれを計算して代入する
    execute if data storage neac: _.MarkerMotion.bounce.e if score #MarkerMotion.Bounce.e neac_value matches 0..999 store result storage neac: _.MarkerMotion.speed.amount int 0.001 run scoreboard players operation #MarkerMotion.Speed neac_value *= #MarkerMotion.Bounce.e neac_value
    execute if data storage neac: _.MarkerMotion.bounce.e if score #MarkerMotion.Bounce.e neac_value matches 0..999 run scoreboard players operation #MarkerMotion.Speed neac_value /= #1000 neac_value
