...
# 今のYベクトル(重力)を取得して次tickのベクトルを引く
    scoreboard players operation #MarkerMotion.Bounce.CG.TMP2 neac_value = #MarkerMotion.Bounce.CG.YVec neac_value
    # ブロック下面
        execute if score #MarkerMotion.BlockCheck neac_value matches 3 if score #MarkerMotion.Bounce.CG.TMP2 neac_value matches 1.. run scoreboard players operation #MarkerMotion.Bounce.CG.TMP2 neac_value *= #-1 neac_value
    # ブロック上面
        execute if score #MarkerMotion.BlockCheck neac_value matches 4 if score #MarkerMotion.Bounce.CG.TMP2 neac_value matches ..-1 run scoreboard players operation #MarkerMotion.Bounce.CG.TMP2 neac_value *= #-1 neac_value

    # 計算
        scoreboard players operation #MarkerMotion.Bounce.CG.TMP1 neac_value += #MarkerMotion.Bounce.CG.TMP2 neac_value
        ...
        # data.MarkerMotion.bounce.eが1のとき値を直接入れる
            execute unless score #MarkerMotion.Bounce.e neac_value matches 1000 store result storage neac: _.MarkerMotion.GravitySum int 1 run scoreboard players get #MarkerMotion.Bounce.CG.TMP1 neac_value
        # data.MarkerMotion.bounce.eが1以外の場合は計算して代入する
            execute if score #MarkerMotion.Bounce.e neac_value matches 0..999 store result storage neac: _.MarkerMotion.GravitySum int 0.001 run scoreboard players operation #MarkerMotion.Bounce.CG.TMP1 neac_value *= #MarkerMotion.Bounce.e neac_value
...
# スコアリセット
    ...
    scoreboard players reset #MarkerMotion.Bounce.CG.TMP2