...
# シュルカー探索
execute unless data storage neac: _.MarkerMotion.stopwith{block:0b} if entity @e[type=shulker,tag=MarkerMotion.shulker,distance=..5,limit=1] positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[type=shulker,tag=MarkerMotion.shulker,dx=0,dy=0,dz=0,limit=1] run data modify storage neac: _.tp[0][].success set value 1b

# エンティティ探査
execute if entity @e[tag=MarkerMotion.target,distance=..5,limit=1] positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[tag=MarkerMotion.target,dx=0,dy=0,dz=0,limit=1] run data modify storage neac: _.tp[0][].success set value 1b
